...
class productController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $product->title = $request->title;
            ...
            if ($request->hasFile('image')) {
                ...
                $path->move(public_path('uploads/products/'), $newPhoto);
            // $product->url = $path;
        ...
        $product=product::query()->create([
            ...
            'views'=>'0',
            // 'url'=>'5'
            'url'=>asset("uploads/products/$newPhoto"),
          //  'url'=>$request->url,
        ]);
        // $product = new product;
        // $product->title = $request->title;
    ...
    public function destroyMyAccount($id)
    {
        ...
        $user->delete();
        return $this->sendResponse(null, 'User Deleted successullly ');
        }
    }
    public function searchByName( Request $request)
    {
        $key=$request->pro_name;
      $data=product::
      where('pro_name','like','%'.$key.'%')
      ->get();
      return $this->sendResponse('search By Name',$data);
    }

    public function searchByCategory( Request $request)
    {
        $key=$request->pro_Category;
      $data=product::
      where('pro_Category','like','%'.$key.'%')
      ->get();
    ...
    {
        $key=$request->pro_expiration_Date;
      $data=product::
      where('pro_expiration_Date','like','%'.$key.'%')
      ->get();
